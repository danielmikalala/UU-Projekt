...
export default function ManagedCampaignList() {
  ...
  const filteredPendingCampaigns = campaigns.filter(
    (c) => c.status === "PendingApproval",
  );
  const filteredApprovedCampaigns = campaigns.filter(
    (c) => c.status === "Approved",
  );
        ...
        <div className="w-full lg:mx-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredApprovedCampaigns.length > 0 &&
              filteredApprovedCampaigns.map((c, index) => (
                <FundingCard
                  key={index}
                  id={c._id}
                  title={c.name}
        ...
        <div className="w-full lg:mx-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPendingCampaigns.length > 0 &&
              filteredPendingCampaigns.map((c, index) => (
                <FundingCard
                  key={index}
                  id={c._id}
                  title={c.name}
